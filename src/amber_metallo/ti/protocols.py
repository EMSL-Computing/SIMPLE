...
from amber_metallo.ti.config import TIDecouplingMode, TIImplementationMode, TIProtocolConfig
...
def _water_ref_minimization_lines(*, settings: InheritedMDSettings) -> list[str]:
    return [
        "&cntrl",
        "  imin = 1,",
        "  ntx = 1,",
...
def generate_water_reference_preparation_inputs(
    *,
    ...
    stage_specs = [
        (
            ...
            _render_water_ref_stage(
                ...
                lines=_water_ref_minimization_lines(settings=inherited_settings),
            ),
        ),
        (
            "02_heat.in",
    ...
    for filename, title, start_source, writes_trajectory, content in stage_specs:
        ...
        stages.append(
            PreparationStage(
                filename=str(Path("prep") / "inputs" / filename),
                title=title,
...
def generate_bound_start_preparation_inputs(
    *,
    ...
    stages = [
        PreparationStage(
            filename=str(Path("bound_start_prep") / "inputs" / "01_min.in"),
        ...
        PreparationStage(
            filename=str(Path("bound_start_prep") / "inputs" / "02_eq.in"),
...
def generate_qoff_endpoint_preparation_inputs(
    *,
    ...
    stages = [
        PreparationStage(
            filename=str(Path("qoff_endpoint_prep") / "inputs" / "01_min.in"),
        ...
        PreparationStage(
            filename=str(Path("qoff_endpoint_prep") / "inputs" / "02_eq.in"),
...
def generate_ti_inputs(
    *,
    ...
    qoff_dir = output_dir / "qoff" / "inputs"
    ...
    use_single_topology_gti_decoupling = _uses_single_topology_gti_decoupling(config)
    qoff_lambdas = (
    ...
    for index, clambda in enumerate(qoff_lambdas, start=1):
        ...
        windows.append(
            TIWindow(
                filename=str(Path("qoff") / "inputs" / filename),
                title=title,
                phase="qoff",
                clambda=clambda,
                start_source=window_start_source,
    ...
    if use_single_topology_gti_decoupling:
        write_json(output_dir / "ti_manifest.json", {"windows": [window.to_dict() for window in windows]})
        return windows
    ...
    for index, clambda in enumerate(config.vdw_lambdas, start=1):
        ...
        windows.append(
            TIWindow(
                filename=str(Path("vdwoff") / "inputs" / filename),
                title=title,
                phase="vdwoff",
                clambda=clambda,
                start_source="qoff_endpoint",
